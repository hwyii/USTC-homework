...
def plot_test():
    ...
    X = pd.read_table("Aggregation.txt",sep = ' ',header=None)
    X = X.values
...
def plot_diff_eps():
    #X = np.concatenate((X1, X2))
    X,c = pd.read_table("Aggregation.txt",sep = ' ',header=None)
    X = X.values
...
def plot_fig():
    ...
    @return     { description_of_the_return_value }
    """
    data_mat,c = pd.read_table("Aggregation.txt",sep = ' ',header=None)
